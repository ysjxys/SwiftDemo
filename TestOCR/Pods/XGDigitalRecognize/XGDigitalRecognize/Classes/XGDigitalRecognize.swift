...
//  Copyright © 2017年 XGNet. All rights reserved.
//
...
public var globalNetwork = FFNN.fromFile(Bundle(for: XGDigitalRecognize.self).url(forResource: "OCR-Network", withExtension: nil)!)!

open class XGDigitalRecognize {
    ...
    public   init(image: UIImage, delegate: SwiftOCRDelegate?, _ completionHandler: @escaping (String, UIImage) -> Void){
    ...
    open   func recognize(_ image: UIImage, _ completionHandler: @escaping (String, UIImage) -> Void){
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for blob in blobs {
                do {
                    ...
                    for networkResultIndex in 0..<networkResult.count {
                        ...
                        guard characterConfidence >= ocrRecognizedBlobConfidenceThreshold else {
                            continue
                        }
            ...
            completionHandler(recognizedString, image)
        }
    }
    ...
    open   func recognizeInRect(_ image: UIImage, rect: CGRect, completionHandler: @escaping (String, UIImage) -> Void){
        DispatchQueue.global(qos: .userInitiated).async {
            #if os(iOS)
