...
class YSJGradientLabel: UILabel {
    ...
    private func createFlashText() {
        ...
        if let size = text?.size(attributes: [NSFontAttributeName: font, NSParagraphStyleAttributeName: style]) {
        ...
        text?.draw(in: CGRect(x: 0, y: topDistance, width: bounds.width, height: bounds.height), withAttributes: [NSFontAttributeName: font, NSParagraphStyleAttributeName: style])
