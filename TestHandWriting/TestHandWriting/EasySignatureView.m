...
//    self.backgroundColor = [UIColor whiteColor];
    ...
    if(!isSure && !isHaveDraw)
    {
        ...
        UILabel *tempLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, CGFLOAT_MAX, 20)];
        ...
        CGFloat width = tempLabel.frame.size.width;
        CGFloat height = tempLabel.frame.size.height;
        
        CGContextSetRGBFillColor (context,  199/255.0, 199/255.0,199/255.0, 1.0);//设置填充颜色
        CGRect rect1 = CGRectMake((rect.size.width -width)/2, (rect.size.height -height)/3-5,width, height);
        origionX = rect1.origin.x;
        totalWidth = rect1.origin.x+width;
